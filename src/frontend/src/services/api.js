...
export async function retrySectionAnalysis(payload) {
  const res = await fetch(`${API_BASE}/api/retry-section-analysis`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  ...
  const data = await res.json();
  if (!res.ok) {
    throw new Error(data.error || data.detail || "Failed to regenerate section analysis");
  }
  return data;
}
